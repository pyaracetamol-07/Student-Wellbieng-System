...
function saveData(data) {
  ...
  const satisfaction = Number(data.satisfaction); // NEW
  ...
  const riskScore =
    sad + academic + lonely + exhausted + hopefulRev + sleepRev + mood;
  ...
  return {
    ...
    responses: {
      Sad: sad,
      Academic: academic,
      Satisfaction: satisfaction, // NEW in graph
      Lonely: lonely,
      Exhausted: exhausted,
      Hopeful: hopeful,
      Sleep: sleep,
